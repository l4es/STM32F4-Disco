...
error_t asn1ReadTag(const uint8_t *data, size_t length, Asn1Tag *tag)
{
   ...
   else
   {
      ...
      for(i = 1; ; i++)
      {
         ...
         tag->length = (tag->length << 7) | (data[i] & 0x7F);
...
error_t asn1CheckOid(const Asn1Tag *tag, const uint8_t *oid, size_t length)
{
   ...
   if(memcmp(tag->value, oid, length))
      return ERROR_WRONG_IDENTIFIER;
...
error_t asn1DumpObject(const uint8_t *data, size_t length, uint_t level)
{
   ...
   //Successful decoding of the ASN.1 object
   return NO_ERROR;
}
...
void asn1DumpOid(const uint8_t *data, size_t length)
{
   ...
   TRACE_DEBUG("%u.%u", data[0] / 40, data[0] % 40);
