...
error_t dsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext, const DsaPrivateKey *key,
   const uint8_t *digest, size_t digestLength, DsaSignature *signature)
{
   error_t error;
   uint_t n;
   Mpi k;
   ...
   const uint8_t *digest, size_t digestLength, const DsaSignature *signature)
{
   ...
   MPI_CHECK(mpiExpMod(&u1, &key->g, &u1, &key->p));
   MPI_CHECK(mpiExpMod(&u2, &key->y, &u2, &key->p));
   MPI_CHECK(mpiMulMod(&v, &u1, &u2, &key->p));
