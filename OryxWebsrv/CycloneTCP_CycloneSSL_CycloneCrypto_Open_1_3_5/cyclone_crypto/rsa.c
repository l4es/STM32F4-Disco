   ...
   const uint8_t *digest, const uint8_t *signature, size_t signatureLength)
{
   ...
   do
   {
      ...
      if(oidLength != hash->oidSize || memcmp(oid, hash->oid, oidLength))
         break;
   ...
   const uint8_t *digest, uint8_t *em, size_t emLength)
{
   ...
   em[i++] = hash->oidSize + hash->digestSize + 8;
   em[i++] = ASN1_ENCODING_CONSTRUCTED | ASN1_TYPE_SEQUENCE;
   em[i++] = hash->oidSize + 4;
   ...
   size_t *oidLength, const uint8_t **digest, size_t *digestLength)
{
   ...
   if(emLength < 11) return ERROR_INVALID_LENGTH;

   //The first octet of EM must have a value of 0x00
   if(em[0] != 0x00) return ERROR_UNEXPECTED_VALUE;
   //The block type BT shall be 0x01
   if(em[1] != 0x01) return ERROR_UNEXPECTED_VALUE;
   ...
   for(i = 2; i < emLength; i++)
   {
      ...
      if(em[i] != 0xFF) return ERROR_INVALID_PADDING;
   }

   //Check whether the padding string is properly terminated
   if(i >= emLength) return ERROR_INVALID_PADDING;
   //The length of PS cannot be less than 8 octets
   if(i < 10) return ERROR_INVALID_PADDING;
   ...
   if(error || !tag.constructed || tag.type != ASN1_TYPE_SEQUENCE)
   ...
   //Failed to decode the ASN.1 tag?
   if(error || tag.constructed || tag.type != ASN1_TYPE_OBJECT_IDENTIFIER)
      return ERROR_INVALID_TAG;
   ...
   *oidLength = tag.length;
   ...
   error = asn1ReadTag(data, length, &tag);
   //Failed to decode the ASN.1 tag?
   if(error || tag.constructed || tag.type != ASN1_TYPE_OCTET_STRING)
      return ERROR_INVALID_TAG;
