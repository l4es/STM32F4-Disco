...
int_t socket(int_t family, int_t type, int_t protocol)
{
   ...
   //Create a socket
   socket = socketOpen(type, protocol);
...
#if (IPV4_SUPPORT == ENABLED)
   //IPv4 address?
   if(socket->localIpAddr.length == sizeof(Ipv4Addr) && *addrlen >= sizeof(sockaddr_in))
   {
      //Point to the IPv4 address information
      sockaddr_in *sa = (sockaddr_in *) addr;
      ...
      //Set address family and port number
      sa->sin_family = AF_INET;
      sa->sin_port = htons(socket->localPort);
      //Copy IPv4 address
      sa->sin_addr.s_addr = socket->localIpAddr.ipv4Addr;
...
#if (IPV6_SUPPORT == ENABLED)
   //IPv6 address?
   if(socket->localIpAddr.length == sizeof(Ipv6Addr) && *addrlen >= sizeof(sockaddr_in6))
   {
      //Point to the IPv6 address information
      sockaddr_in6 *sa = (sockaddr_in6 *) addr;
      ...
      //Set address family and port number
      sa->sin6_family = AF_INET6;
      sa->sin6_port = htons(socket->localPort);
      //Copy IPv6 address
      ipv6CopyAddr(sa->sin6_addr.s6_addr, &socket->localIpAddr.ipv6Addr);
   ...
   if(level == SOL_SOCKET)
   {
      ...
      case SO_RCVTIMEO:
         ...
         if(!t->tv_sec && !t->tv_usec)
            ...
            socket->timeout = t->tv_sec * 1000 + t->tv_usec / 1000;
...
int_t getsockopt(int_t s, int_t level, int_t optname, void *optval, int_t *optlen)
{
   ...
   if(level == SOL_SOCKET)
   {
      ...
      case SO_RCVTIMEO:
         ...
         {
            t->tv_sec = socket->timeout / 1000;
            t->tv_usec = (socket->timeout % 1000) * 1000;
         }
      ...
      case SO_ERROR:
         //Check option length
         if(*optlen < sizeof(socket->lastError))
         {
            socketError(NULL, ERROR_INVALID_LENGTH);
            return SOCKET_ERROR;
         }
         //Copy timeout value
         memcpy(optval, &socket->lastError, sizeof(socket->lastError));
         //Return the actual length of the option
         *optlen = sizeof(socket->lastError);
         //Successful processing
         break;
