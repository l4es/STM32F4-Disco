...
void ethProcessFrame(NetInterface *interface, EthHeader *ethFrame, size_t length)
{
   ...
   //Debug message
   TRACE_DEBUG("Ethernet frame received (%u bytes)...\r\n", length);
   ...
   case ETH_TYPE_IPV4:
      ...
      ipv4ProcessPacket(interface, &ethFrame->srcAddr, (Ipv4Header *) ethFrame->data, length);
      break;
#endif
#if (IPV6_SUPPORT == ENABLED)
   ...
   case ETH_TYPE_IPV6:
      ...
      ipv6ProcessPacket(interface, &ethFrame->srcAddr, (ChunkedBuffer *) &buffer);
      break;
#endif
   ...
   ChunkedBuffer *buffer, size_t offset, uint16_t type)
{
   ...
   //Debug message
   TRACE_DEBUG("Sending Ethernet frame (%u bytes)...\r\n", length);
   //Dump Ethernet header contents for debugging purpose
...
error_t ethCheckDestAddr(NetInterface *interface, const MacAddr *macAddr)
{
   ...
   if(macCompAddr(macAddr, &interface->macAddr))
      return NO_ERROR;
   //Broadcast MAC address?
   if(macCompAddr(macAddr, &MAC_BROADCAST_ADDR))
      return NO_ERROR;
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      if(macCompAddr(&interface->macFilter[i].addr, macAddr))
      {
         ...
         //The specified MAC address is acceptable
         return NO_ERROR;
      }
   }
   ...
   //Debug message
   TRACE_WARNING("Wrong destination MAC address!\r\n");
