   ...
   IpPseudoHeader *pseudoHeader, const ChunkedBuffer *buffer, size_t offset)
{
   ...
   //Debug message
   TRACE_INFO("UDP datagram received (%u bytes)...\r\n", length);
   ...
   queueItem->remotePort = ntohs(header->srcPort);
   ...
   if(pseudoHeader->length == sizeof(Ipv4PseudoHeader))
   {
      ...
      queueItem->remoteIpAddr.length = sizeof(Ipv4Addr);
      queueItem->remoteIpAddr.ipv4Addr = pseudoHeader->ipv4Data.srcAddr;
   }
#endif
#if (IPV6_SUPPORT == ENABLED)
   //IPv6 remote address?
   if(pseudoHeader->length == sizeof(Ipv6PseudoHeader))
   {
      ...
      queueItem->remoteIpAddr.length = sizeof(Ipv6Addr);
      queueItem->remoteIpAddr.ipv6Addr = pseudoHeader->ipv6Data.srcAddr;
   }
#endif
   ...
   uint16_t destPort, const void *data, size_t length, size_t *written)
{
   ...
   //Allocate a memory buffer to hold the UDP header and the payload
   buffer = ipAllocBuffer(sizeof(UdpHeader), &offset);
   ...
   do
   {
      ...
      //Calculate the length of the complete datagram
      length += sizeof(UdpHeader);
      ...
      //Format UDP header
      header->srcPort = htons(socket->localPort);
   ...
   uint16_t *remotePort, void *data, size_t size, size_t *received, uint_t flags)
{
   ...
   *received = chunkedBufferRead(data, queueItem->buffer, queueItem->offset, size);
   ...
   //Save the IP address of the peer and the corresponding port number
   if(remoteIpAddr)
      *remoteIpAddr = queueItem->remoteIpAddr;
   if(remotePort)
      *remotePort = queueItem->remotePort;
...
void udpDumpHeader(const UdpHeader *datagram)
{
   ...
   TRACE_DEBUG("  Source Port = %u\r\n", ntohs(datagram->srcPort));
   TRACE_DEBUG("  Destination Port = %u\r\n", ntohs(datagram->destPort));
