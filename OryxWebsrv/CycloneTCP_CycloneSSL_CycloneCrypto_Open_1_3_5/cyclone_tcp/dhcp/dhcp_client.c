...
error_t dhcpSendDiscover(DhcpClientCtx *context)
{
   ...
   //Debug message
   TRACE_INFO("\r\n%s: Sending DHCP message (%u bytes)...\r\n",
      timeFormat(osGetTickCount()), DHCP_MSG_MIN_SIZE);
...
error_t dhcpParseOffer(DhcpClientCtx *context, size_t length)
{
   ...
   //Debug message
   TRACE_INFO("\r\n%s: DHCP message received (%u bytes)...\r\n",
...
void dhcpDumpConfig(DhcpClientCtx *context)
{
   ...
   //Display DNS servers
   if(context->interface->ipv4Config.dnsServerCount >= 1)
      TRACE_INFO("  Primary DNS = %s\r\n", ipv4AddrToString(context->interface->ipv4Config.dnsServer[0], NULL));
   if(context->interface->ipv4Config.dnsServerCount >= 2)
      TRACE_INFO("  Secondary DNS = %s\r\n", ipv4AddrToString(context->interface->ipv4Config.dnsServer[1], NULL));
}
