...
error_t dhcpDumpMessage(const DhcpMessage *message, size_t length)
{
   ...
   TRACE_DEBUG("  Op Code (op) = %u (%s)\r\n", message->op, label);
   TRACE_DEBUG("  Hardware Type (htype) = %u\r\n", message->htype);
   TRACE_DEBUG("  Hardware Address Length (hlen) = %u\r\n", message->hlen);
   TRACE_DEBUG("  Hops (hops) = %u\r\n", message->hops);
   TRACE_DEBUG("  Transaction ID (xid) = 0x%08X\r\n", ntohl(message->xid));
   TRACE_DEBUG("  Seconds (secs) = %us\r\n", ntohs(message->secs));
   TRACE_DEBUG("  Flags (flags) = 0x%04X\r\n", ntohs(message->flags));
   ...
   TRACE_DEBUG("  Magic Cookie = 0x%08X\r\n", ntohl(message->magicCookie));
   ...
   for(i = 0; i < length; i++)
   {
      ...
      if(option->code < arraysize(optionLabel))
         TRACE_DEBUG("  %s option (%u bytes)\r\n", optionLabel[option->code], option->length);
      else
         TRACE_DEBUG("  Option %u (%u bytes)\r\n", option->code, option->length);
...
error_t dhcpDumpInt16(const DhcpOption *option)
{
   ...
   //Dump option contents
   TRACE_DEBUG("    %u\r\n", (option->value[0] << 8) | option->value[1]);
