...
error_t dhcpv6ClientStart(Dhcpv6ClientCtx *context, const Dhcpv6ClientSettings *settings)
{
   error_t error;
   Dhcpv6DuidLl *duid;
   OsTask *task;
   ...
   Dhcpv6FormatCallback formatRequest, Dhcpv6ParseCallback parseResponse)
{
   ...
   context->transactionId = rand() & 0x00FFFFFF;
   ...
   for(context->rc = 0; !context->mrc || context->rc < context->mrc; context->rc++)
   {
      ...
      //Debug message
      TRACE_INFO("\r\n%s: Sending DHCPv6 message (%u bytes)...\r\n",
...
error_t dhcpv6WaitForResponse(Dhcpv6ClientCtx *context,
   Dhcpv6ParseCallback parseResponse, time_t timeout)
{
   error_t error;
   size_t length;
   time_t startTime;
   ...
   while(elapsedTime < timeout)
   {
      ...
      if(eventDesc.eventFlags & SOCKET_EVENT_RX_READY)
      {
         ...
         //Debug message
         TRACE_INFO("\r\n%s: DHCPv6 message received (%u bytes)...\r\n",
            timeFormat(osGetTickCount()), length);
         //Dump the contents of the message for debugging purpose
         dhcpv6DumpMessage(message, length);
...
error_t dhcpv6FormatSolicit(Dhcpv6ClientCtx *context, Dhcpv6Message *message, size_t *length)
{
   ...
   *length = sizeof(Dhcpv6Message);
   ...
   iaNaOption.iaId = htonl(context->interface->identifier);
   iaNaOption.t1 = 0;
   iaNaOption.t2 = 0;
...
error_t dhcpv6ParseIaNaOption(Dhcpv6ClientCtx *context, const Dhcpv6Option *option)
{
   ...
   if(ntohl(iaNaOption->iaId) != context->interface->identifier)
      return ERROR_INVALID_OPTION;
...
void dhcpv6DumpConfig(Dhcpv6ClientCtx *context)
{
   ...
   TRACE_INFO("  Lease Start Time = %s\r\n", timeFormat(context->leaseStartTime));
   TRACE_INFO("  T1 = %lus\r\n", context->t1);
   TRACE_INFO("  T2 = %lus\r\n", context->t2);
   TRACE_INFO("  Preferred Lifetime = %lus\r\n", context->preferredLifetime);
   TRACE_INFO("  Valid Lifetime = %lus\r\n", context->validLifetime);
