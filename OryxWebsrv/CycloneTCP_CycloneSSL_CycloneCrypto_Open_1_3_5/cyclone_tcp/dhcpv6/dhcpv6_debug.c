...
{
   //Dump contents
   TRACE_DEBUG("%sClient Identifier option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG_ARRAY(prefix[level + 1], option->value, ntohs(option->length));
...
error_t dhcpv6DumpIaTaOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sIA_TA option (%u bytes)\r\n", prefix[level], ntohs(option->length));
...
error_t dhcpv6DumpOroOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sOption Request option (%u bytes)\r\n", prefix[level], ntohs(option->length));
...
error_t dhcpv6DumpPreferenceOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sPreference option (%u bytes)\r\n", prefix[level], ntohs(option->length));
...
error_t dhcpv6DumpElapsedTimeOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sElapsed Time option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG("%s%ums\r\n", prefix[level + 1], ntohs(elapsedTimeOption->value) * 10);
...
error_t dhcpv6DumpStatusCodeOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sStatus Code option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG("%sCode = %u (%s)\r\n", prefix[level + 1], code, label);
...
error_t dhcpv6DumpRapidCommitOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sRapid Commit option (%u bytes)\r\n", prefix[level], ntohs(option->length));
...
{
   //Dump contents
   TRACE_DEBUG("%sUser Class option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG_ARRAY(prefix[level + 1], option->value, ntohs(option->length));
...
{
   //Dump contents
   TRACE_DEBUG("%sVendor Class option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG_ARRAY(prefix[level + 1], option->value, ntohs(option->length));
...
{
   //Dump contents
   TRACE_DEBUG("%sInterface ID option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG_ARRAY(prefix[level + 1], option->value, ntohs(option->length));
...
error_t dhcpv6DumpReconfMessageOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sReconfigure Message option (%u bytes)\r\n", prefix[level], ntohs(option->length));
   TRACE_DEBUG("%sMessage Type = %u\r\n", prefix[level + 1], reconfMessageOption->msgType);
...
error_t dhcpv6DumpReconfAcceptOption(const Dhcpv6Option *option, uint_t level)
{
   ...
   //Dump contents
   TRACE_DEBUG("%sReconfigure Accept option (%u bytes)\r\n", prefix[level], ntohs(option->length));
