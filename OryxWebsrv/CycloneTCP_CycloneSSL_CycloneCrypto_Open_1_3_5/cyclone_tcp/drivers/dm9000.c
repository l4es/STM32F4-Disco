...
error_t dm9000Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("  VID = 0x%04X\r\n", vendorId);
   TRACE_INFO("  PID = 0x%04X\r\n", productId);
   TRACE_INFO("  CHIPR = 0x%02X\r\n", chipRevision);
   TRACE_INFO("  PHYIDR1 = 0x%04X\r\n", dm9000ReadPhyReg(DM9000_PHY_REG_PHYIDR1));
   TRACE_INFO("  PHYIDR2 = 0x%04X\r\n", dm9000ReadPhyReg(DM9000_PHY_REG_PHYIDR2));
...
error_t dm9000SetMacFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("  MAR = %02X %02X %02X %02X %02X %02X %02X %02X\r\n",
...
uint32_t dm9000CalcCrc(const void *data, uint_t length)
{
   uint_t i;
   uint_t j;
