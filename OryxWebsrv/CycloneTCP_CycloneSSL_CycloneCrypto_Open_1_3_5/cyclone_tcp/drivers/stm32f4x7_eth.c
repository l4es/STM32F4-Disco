...
//Transmit buffer
static uint8_t txBuffer[STM32F4X7_TX_BUFFER_COUNT][STM32F4X7_TX_BUFFER_SIZE] __attribute__((aligned(4)));
//Receive buffer
static uint8_t rxBuffer[STM32F4X7_RX_BUFFER_COUNT][STM32F4X7_RX_BUFFER_SIZE] __attribute__((aligned(4)));
//Transmit DMA descriptors
static Stm32f4x7TxDmaDesc txDmaDesc[STM32F4X7_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//Receive DMA descriptors
static Stm32f4x7RxDmaDesc rxDmaDesc[STM32F4X7_RX_BUFFER_COUNT] __attribute__((aligned(4)));
...
//STM3240G-EVAL evaluation board?
   ...
   RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN | RCC_AHB1ENR_GPIOBEN | RCC_AHB1ENR_GPIOCEN |
      RCC_AHB1ENR_GPIOFEN | RCC_AHB1ENR_GPIOGEN | RCC_AHB1ENR_GPIOHEN | RCC_AHB1ENR_GPIOIEN;
...
void stm32f4x7EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32F4X7_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32F4X7_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (STM32F4X7_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
void stm32f4x7EthRxEventHandler(NetInterface *interface)
{
   size_t length;
   ...
   if(ETH->DMASR & ETH_DMASR_RS)
   {
      ...
      while(1)
      {
         ...
         length = stm32f4x7EthReceivePacket(interface, interface->ethFrame, ETH_MAX_FRAME_SIZE);
...
error_t stm32f4x7EthSetMacFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < interface->macFilterSize; i++)
   {
      ...
      //The upper 6 bits of in the CRC register are used to index the contents of the hash table
      k = (crc >> 26) & 0x3F;
   ...
   //Debug message
   TRACE_INFO("  MACHTLR = %08lX\r\n", ETH->MACHTLR);
   TRACE_INFO("  MACHTHR = %08lX\r\n", ETH->MACHTHR);
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   if(length > STM32F4X7_TX_BUFFER_SIZE)
   {
...
size_t stm32f4x7EthReceivePacket(NetInterface *interface,
