...
error_t igmpInit(NetInterface *interface)
{
   ...
   interface->igmpv1RouterPresentTimer =
      osGetTickCount() + IGMP_V1_ROUTER_PRESENT_TIMEOUT;
...
error_t igmpJoinGroup(NetInterface *interface, Ipv4FilterEntry *entry)
{
   ...
   else
   {
      ...
      if(interface->linkState)
      {
         ...
         entry->flag = TRUE;
         //Start timer
         entry->timer = osGetTickCount() + IGMP_UNSOLICITED_REPORT_INTERVAL;
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   //Debug message
   TRACE_INFO("IGMP message received (%u bytes)...\r\n", length);
...
error_t igmpSendReportMessage(NetInterface *interface, Ipv4Addr ipAddr)
{
   ...
   //Debug message
   TRACE_INFO("Sending IGMP message (%u bytes)...\r\n", sizeof(IgmpMessage));
   //Dump message contents for debugging purpose
   igmpDumpMessage(message);
...
{
   //Dump IGMP message
   TRACE_DEBUG("  Type = 0x%02X\r\n", message->type);
   TRACE_DEBUG("  Max Resp Time = 0x%02X\r\n", message->maxRespTime);
   TRACE_DEBUG("  Checksum = 0x%04X\r\n", ntohs(message->checksum));
   TRACE_DEBUG("  Group Address = 0x%04X\r\n", ipv4AddrToString(message->groupAddr, NULL));
}
