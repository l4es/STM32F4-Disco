   ...
   const ChunkedBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   //Debug message
   TRACE_INFO("ICMPv6 message received (%u bytes)...\r\n", length);
   ...
   const ChunkedBuffer *request, size_t requestOffset)
{
   ...
   //Debug message
   TRACE_INFO("ICMPv6 Echo Request message received (%u bytes)...\r\n", requestLength);
   //Dump message contents for debugging purpose
   ...
   //Debug message
   TRACE_INFO("Sending ICMPv6 Echo Reply message (%u bytes)...\r\n", replyLength);
   //Dump message contents for debugging purpose
   ...
   uint8_t code, uint32_t parameter, const ChunkedBuffer *ipPacket)
{
   ...
   //Debug message
   TRACE_INFO("Sending ICMPv6 Error message (%u bytes)...\r\n", length);
   //Dump message contents for debugging purpose
...
void icmpv6DumpEchoMessage(const Icmpv6EchoMessage *message)
{
   ...
   TRACE_DEBUG("  Code = %u\r\n", message->code);
   TRACE_DEBUG("  Checksum = 0x%04X\r\n", ntohs(message->checksum));
   TRACE_DEBUG("  Identifier = 0x%04X\r\n", ntohs(message->identifier));
   TRACE_DEBUG("  Sequence Number = 0x%04X\r\n", ntohs(message->sequenceNumber));
}
