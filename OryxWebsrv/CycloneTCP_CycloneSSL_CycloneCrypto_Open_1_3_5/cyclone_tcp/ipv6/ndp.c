   ...
   const ChunkedBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   //Debug message
   TRACE_INFO("Neighbor Solicitation message received (%u bytes)...\r\n", length);
   ...
   const ChunkedBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   //Debug message
   TRACE_INFO("Neighbor Advertisement message received (%u bytes)...\r\n", length);
   //Dump message contents for debugging purpose
...
error_t ndpSendRouterSol(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Sending Router Solicitation message (%u bytes)...\r\n", length);
   //Dump message contents for debugging purpose
...
error_t ndpSendNeighborSol(NetInterface *interface, const Ipv6Addr *targetIpAddr)
{
   ...
   //Debug message
   TRACE_INFO("Sending Neighbor Solicitation message (%u bytes)...\r\n", length);
   //Dump message contents for debugging purpose
   ndpDumpNeighborSolMessage(message);
   ...
   const Ipv6Addr *targetIpAddr, const Ipv6Addr *destIpAddr)
{
   ...
   //Debug message
   TRACE_INFO("Sending Neighbor Advertisement message (%u bytes)...\r\n", length);
   //Dump message contents for debugging purpose
   ndpDumpNeighborAdvMessage(message);
...
void ndpDumpRouterAdvMessage(const NdpRouterAdvMessage *message)
{
   ...
   TRACE_DEBUG("  Code = %u\r\n", message->code);
   TRACE_DEBUG("  Checksum = 0x%04X\r\n", ntohs(message->checksum));
   TRACE_DEBUG("  Cur Hop Limit = %u\r\n", message->curHopLimit);
   TRACE_DEBUG("  M = %u\r\n", message->m);
   TRACE_DEBUG("  O = %u\r\n", message->o);
   TRACE_DEBUG("  Router Lifetime = %u\r\n", ntohs(message->routerLifetime));
   TRACE_DEBUG("  Reachable Time = %u\r\n", ntohl(message->reachableTime));
   TRACE_DEBUG("  Retrans Timer = %u\r\n", ntohl(message->retransTimer));
}
