...
error_t tcpDiscardStart(void)
{
   ...
   do
   {
      ...
      task = osTaskCreate("TCP Discard Listener", tcpDiscardListenerTask,
...
void tcpDiscardListenerTask(void *param)
{
   ...
   while(1)
   {
      ...
      //Debug message
      TRACE_INFO("Discard service: connection established with client %s port %u\r\n",
      ...
      context = osMemAlloc(sizeof(DiscardServiceContext));
      ...
      task = osTaskCreate("TCP Discard Connection", tcpDiscardConnectionTask,
...
void tcpDiscardConnectionTask(void *param)
{
   ...
   //Debug message
   TRACE_INFO("Discard service: %u bytes received in %lu ms (%lu kBps, %lu kbps)\r\n",
...
error_t udpDiscardStart(void)
{
   ...
   do
   {
      ...
      task = osTaskCreate("UDP Discard", udpDiscardTask,
         context, DISCARD_SERVICE_STACK_SIZE, DISCARD_SERVICE_PRIORITY);
...
void udpDiscardTask(void *param)
{
   ...
   while(1)
   {
      ...
      {
         //Debug message
         TRACE_INFO("Discard service: %u bytes received from %s port %u\r\n",
