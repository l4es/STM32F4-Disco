...
*/
/********************************************************************************/
#ifndef __SDIO_STM32F4_H
#define __SDIO_STM32F4_H	0x1100
...
/* DMA MODE or FIFO-POLLING Mode */
//#define SD_DMA_MODE
#define SD_POLLING_MODE

/* SDIO DMA goes fail on transferring large amount of data with I2S-Circuler DMA! */
#if defined(SD_DMA_MODE) && \
    (defined(USE_MP3_HELIX) || defined(USE_AAC_HELIX) || defined(USE_WAVE_OUT))
 #warning "Changed SDIO mode DMA into FIFO-Polling"
 #undef SD_DMA_MODE
 #define SD_POLLING_MODE /* Safe and Fast Operation */
#endif

/* Uncomment the following line to select the SD Nomal/High Speed Mode */ 
/* Notice !
   STM32F40x and STM32F41x errata sheet revision5 says 
�@ "SDIO clock divider BYPASS mode may not work properly" and
   "Data corruption in SDIO clock dephasing (NEGEDGE) mode".
   This means CANNOT set 48MHz direct clock anymore.
   
   To handling highspeedmode stably,SDIO_CK should set below "37.5MHz".
   To put it concretely,should be set to below sequance.
   SDIOCLK(75MHz)/2 = SDIO_CK(37.5MHz).
   SDIO clock divider BYPASS mode set to"DISABLE".
   SDIO clock polarity set to "RISING EDGE".
...
/** 
  ...
  * For STM32F40x/41x :48MHz(SDIOCLK MAX Value@SYSCLK168MHz)/(128+2) = 369kHz.
  * For STM32F42x/43x :51MHz(SDIOCLK MAX Value@SYSCLK180MHz)/(128+2) = 392kHz.
  */
#define SDIO_INIT_CLK_DIV                	((uint8_t)0x80)
/** 
  * @brief  SDIO Data Transfer Frequency (25MHz max in NomalMode) 
  * For STM32F40x/41x :48MHz(SDIOCLK MAX Value@SYSCLK168MHz)/(0+2) = 24MHz.
  * For STM32F42x/43x :51MHz(SDIOCLK MAX Value@SYSCLK180MHz)/(0+2) = 25.5MHz.
  */
...
#define SD_SDIO_DMA_CLK               		RCC_AHB1Periph_DMA2
...
#if defined(SD_SDIO_DMA_STREAM3)
 ...
 #define SD_SDIO_DMA_xISR					DMA2->LISR
 ...
 #define SD_SDIO_DMA_xISR					DMA2->HISR
