...
static inline size_t read_file (FIL  *file, uint8_t *buf, uint32_t sizeofbuf)
{
  FRESULT fre=FR_OK;
  static size_t BytesReadfile;
  fre=f_read(file, buf , sizeofbuf, &BytesReadfile);
  return BytesReadfile; 
}

static inline size_t write_file (FIL  *file, uint8_t *buf, uint32_t sizeofbuf)
{
