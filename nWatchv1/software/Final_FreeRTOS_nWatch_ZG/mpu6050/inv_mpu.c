...
u8 i2c_write(uint8_t slave_addr, uint8_t reg_addr, uint8_t length, uint8_t const *data)
{
	slave_addr=0xD0;
		...
		I2C_Send7bitAddress(MPU6050_I2C, slave_addr, I2C_Direction_Transmitter);
...
u8 i2c_read(uint8_t slave_addr, uint8_t reg_addr, uint8_t length, uint8_t *data)
{
	slave_addr=0xD0;
	    ...
	    I2C_Send7bitAddress(MPU6050_I2C, slave_addr, I2C_Direction_Receiver);
 ...
 *  @return     0 if successful.
 */
uint8_t mpu_load_firmware(uint16_t length, const unsigned char *firmware,
