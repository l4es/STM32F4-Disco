...
uint8_t mpu_lp_motion_interrupt(uint16_t thresh, uint8_t time,uint8_t lpa_freq);
...
uint8_t mpu_load_firmware(uint16_t length, const unsigned char *firmware, uint16_t start_addr, uint16_t sample_rate);
